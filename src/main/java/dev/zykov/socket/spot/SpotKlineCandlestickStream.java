...
public abstract class SpotKlineCandlestickStream implements AutoCloseable {
    ...
    public void onOpen(String symbol, WebSocketSession session, HttpRequest request) {
        ...
        this.symbol = symbol;
        log.info("Open spot klineCandlestick for {}", symbol);
    }

    @OnMessage
    public void onMessage(KlineCandlestickModel message) {
        socketCache.addSpotKlineCandlestick(message);
    }

    @OnClose
    public void onClose() {
        log.info("Close spot klineCandlestick for {}", symbol);
    }
